...
<body class="pb-12">
    ...
    <script>
        ...
        function switchTab(tabId) {
            ["adjust", "history", "chart"].forEach(t => {
                document.getElementById(`tabContent-${t}`).classList.add("hidden");
                document.getElementById(`tabBtn-${t}`).className = "flex-1 py-3 text-center border-b-2 border-transparent text-slate-400 hover:text-slate-200 transition";
            });
            document.getElementById(`tabContent-${tabId}`).classList.remove("hidden");
            document.getElementById(`tabBtn-${tabId}`).className = "flex-1 py-3 text-center border-b-2 border-blue-500 font-semibold text-blue-500 transition";
